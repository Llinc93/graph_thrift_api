...
class Neo4jClient(object):
    ...
    def get_ent_graph_g_v4(self, entname, level, node_type, relationshipFilter):
        ...
        '''
        flag = True
        command = "match (n:%s {NAME: '%s'}) call apoc.path.expand(n, '%s', '', 1, %s) yield path foreach(n in nodes(path) | set n.extendnumber=size((n) -[]-> ())) foreach(r in relationships(path) | set r.pid=properties(startNode(r))['ID']) foreach(r in relationships(path) | set r.id=properties(endNode(r))['ID']) foreach(r in relationships(path) | set r.labe=type(r)) foreach(r in relationships(p) | set r.ID=id(r)) foreach(n in nodes(path) | set n.label=labels(n)[0]) return [n in nodes(path) | properties(n)] as n, [r in relationships(path) | properties(r)] as r"
